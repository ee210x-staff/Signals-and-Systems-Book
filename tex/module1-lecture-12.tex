...
\node[coordinate]       (c0) {};	
\node[coordinate,label =below:{$X[n]=\delta[n-k]$},label =above:Input Signal,right=of c0](c1){};	
\node[dspfilter,right= of c1,text height=3em,text width=10em,text depth=3em]                    (c2) {Discrete Time \\LSI System };
\node[coordinate,label=below:{$Y[n]=h[n]$},label = above:\hspace{0.150in}Output Signal,right=of c2]             (c3) {};
